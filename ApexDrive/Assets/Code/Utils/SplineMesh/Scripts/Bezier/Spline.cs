...
namespace SplineMesh {
    ...
    public class Spline : MonoBehaviour {
        ...
        private int m_DistanceCheckPrecisionLevel = 100;
        /// <summary>
        /// The spline nodes.
	...
	public void RefreshCurves() {
        ...
        public Vector3 GetNearestPointOnSpline(Vector3 point)
        {
            ...
            float lowestDistance = 100000.0f;
            for(int i = 0; i < m_DistanceCheckPrecisionLevel; i++)
            {
                Vector3 temp = GetSample((float)i/(float)m_DistanceCheckPrecisionLevel * (nodes.Count-1)).location;
                float distance = Vector3.Distance(temp, point);
