...
public class DetectLeaksWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        _scroll = GUILayout.BeginScrollView(_scroll);
        {
            ...
            {
                _sceneFoldout = EditorGUILayout.Foldout(_sceneFoldout, "<color=" + (_sceneHasMissingScripts ? "red" : "white") + ">Scene GameObjects</color>", _missingFoldoutStyle);
                if (_sceneFoldout)
                    DrawGameObjectList(_sceneList);
 
                _projectFoldout = EditorGUILayout.Foldout(_projectFoldout, "<color=" + (_projectHasMissingScripts ? "red" : "white") + ">Project GameObjects</color>", _missingFoldoutStyle);
                if (_projectFoldout)
                    DrawGameObjectList(_projectList);

                _meshFoldout = EditorGUILayout.Foldout(_meshFoldout, "<color=" + (_projectHasMissingScripts ? "red" : "white") + ">Meshes</color>", _missingFoldoutStyle);
                if(_meshFoldout)
                {
                    foreach (Mesh obj in _meshList)
                    {
                        if (obj == null)
                            continue;
    ...
    private void Scan()
    {
        ...
        _meshList = (FindObjectsOfType(typeof(Mesh)) as Mesh[]).ToList();
